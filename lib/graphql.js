...
const SCHEMA = {
  ...
  types: {
    ...
    Mutation: { kind: 'OBJECT', fields: {
      ...
      updateProfile: { type: 'User', args: { id: 'Int', role: 'String', isAdmin: 'Boolean', points: 'Int' } } // v10 mass assign
    } },
...
function makeResolvers(db, ctx) {
  return {
    ...
    Mutation: {
      ...
      mintPoints: (a) => {
        ctx._priv = (ctx._priv || 0) + 1;             // v10 privileged mutation
      ...
      updateProfile: (a) => {
        ctx._priv = (ctx._priv || 0) + 1;
        // VULN (v10): mass assignment via GraphQL — role/isAdmin trusted, no auth.
        if (a.role != null) db.prepare('UPDATE users SET role = ? WHERE id = ?').run(a.role, a.id);
        if (a.isAdmin != null) db.prepare('UPDATE users SET is_admin = ? WHERE id = ?').run(a.isAdmin ? 1 : 0, a.id);
        if (a.points != null) db.prepare('UPDATE users SET balance_points = ? WHERE id = ?').run(a.points, a.id);
        if (a.role === 'admin' || a.isAdmin) ctx._capstones.push(FLAGS.gql_mass_assign);
        return userRow(db, a.id);
      },
      adminDumpUsers: () => {
        ctx._priv = (ctx._priv || 0) + 1;
...
function resolveSelections(selections, typeName, source, resolvers, db, ctx, meta) {
  ...
  for (const sel of selections) {
    meta.fieldCount = (meta.fieldCount || 0) + 1; // v6: width/alias amplification counter
...
export function executeGraphQL(query, db, ctx = {}) {
  ...
  if (meta.fieldCount > 800) meta.flags.add(FLAGS.burn_gql_amplification); // v6 width bomb
  if (ctx._priv >= 2) meta.flags.add(FLAGS.gql_batch_privesc);             // v10 batch privesc
