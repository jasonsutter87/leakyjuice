...
export const FLAGS = {
  ...
  internal_docs: 'FLAG{lj_internal_docs_exposed}',
  ...
  // ── v10: API boss ──
  gql_mass_assign: 'FLAG{lj_graphql_mass_assignment}',    // updateProfile sets role
  api_inventory_drift: 'FLAG{lj_api_inventory_drift}',    // deprecated /api/v1 still live
  ratelimit_bypass_xff: 'FLAG{lj_ratelimit_xff_bypass}',  // X-Forwarded-For spoof
  gql_batch_privesc: 'FLAG{lj_graphql_batch_privesc}',    // multiple privileged mutations in one doc
  // ── v11: Black Team final boss ──
  black_team: 'FLAG{lj_black_team}',                      // master flag — every persona demonstrated
  // ── v12: Hack the Scoreboard (the instrument is a target) ──
  scoreboard_score_tamper: 'FLAG{lj_scoreboard_score_tamper}',   // client-set score, no recompute
  scoreboard_flag_forgery: 'FLAG{lj_scoreboard_flag_forgery}',   // claim a flag you never earned
  scoreboard_idor: 'FLAG{lj_scoreboard_idor_overwrite}',         // overwrite another player
  scoreboard_xss: 'FLAG{lj_scoreboard_stored_xss}',              // player name rendered raw
  scoreboard_pwned: 'FLAG{lj_scoreboard_pwned}',                 // fraudulent 100% — caught by server verify
  // ── v13: JuicyOps — the internal staff console (left exposed) ──
  internal_console_exposed: 'FLAG{lj_internal_console_exposed}', // reachable, no auth (security by obscurity)
  internal_sql: 'FLAG{lj_internal_sql_console}',                 // arbitrary SQL runner (in-memory DB)
  internal_env: 'FLAG{lj_internal_env_dump}',                    // dumps app secrets
  internal_impersonate: 'FLAG{lj_internal_impersonation}',       // `su <email>` mints a session as anyone
  // ── v14: JuicySlack — leaked workspace export (recon narrative + one live cred) ──
  slack_export_exposed: 'FLAG{lj_slack_export_exposed}',         // full export dropped under public/, robots-advertised
  slack_dm_offboarding: 'FLAG{lj_slack_dm_offboarding}',         // DM excluded from sidebar, present in channels.json
  slack_token_in_chat: 'FLAG{lj_slack_token_in_chat}'           // INTERNAL_TOKEN pasted in chat still authenticates
};
