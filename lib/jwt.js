...
// VULN (v9): verify honours an attacker-supplied `jku` header — fetch keys from any URL and
// trust them. Here the fetched key material is used as the HMAC secret, so an attacker who
// hosts a key they know (e.g. an uploaded file) can forge tokens.
export async function verifyJku(token) {
  try {
    const [h, p, s] = token.split('.');
    const header = JSON.parse(b64urlDecode(h).toString('utf8'));
    const payload = JSON.parse(b64urlDecode(p).toString('utf8'));
    if (!header.jku || header.alg !== 'HS256') return null;
    const key = Buffer.from(await (await fetch(header.jku, { signal: AbortSignal.timeout(3000) })).text());
    const expected = crypto.createHmac('sha256', key).update(`${h}.${p}`).digest();
    const sig = b64urlDecode(s || '');
    if (expected.length === sig.length && crypto.timingSafeEqual(expected, sig)) return { payload, via: 'jku', jku: header.jku };
    return null;
  } catch { return null; }
}
